...
class WP_Review_Me {
    ...
    protected function get_defaults() {
        ...
        $defaults = array(
            ...
            'slug'       => '',
            'rating'     => 5,
            'message'    => sprintf( esc_html__( 'Hey! It&#039;s been a little while that you&#039;ve been using this product. You might not realize it, but user reviews are such a great help to us. We would be so grateful if you could take a minute to leave a review on WordPress.org. Many thanks in advance :)', 'sunny' ) ),
            'link_label' => esc_html__( 'Click here to leave your review', 'sunny' ),
    ...
    private function init() {
        ...
        if ( ! $this->is_wp_compatible() ) {
            $this->spit_error(
                sprintf(
                    esc_html__( 'The library can not be used because your version of WordPress is too old. You need version %s at least.', 'sunny' ),
        ...
        if ( ! $this->is_php_compatible() ) {
            $this->spit_error(
                sprintf(
                    esc_html__( 'The library can not be used because your version of PHP is too old. You need version %s at least.', 'sunny' ),
        ...
        if ( ! function_exists( 'dnh_register_notice' ) ) {
            ...
            if ( ! function_exists( __NAMESPACE__ . '\dnh_register_notice' ) ) {
                $this->spit_error(
                    sprintf(
                        esc_html__( 'Dependencies are missing. Please run a %s.', 'sunny' ),
    ...
    protected function spit_error( $error ) {
        printf(
            ...
            esc_html__( 'WP Review Me Error:', 'sunny' ),
            wp_kses_post( $error )
        );
    }
